...
**`Scripts/Domain/`** is plain C#: the room, the windows, the selection, and the rules about what counts as a
legal opening. **`Scripts/Presentation/`** is everything Unity: views, cameras, input, HUD. Presenters read the
domain; the domain never reads back. Keeping the rules out of `MonoBehaviour` is what makes them testable in
milliseconds instead of by dragging a finger across a wall.
