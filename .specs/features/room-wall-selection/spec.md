...
6. WHEN the user taps a wall row that carries at least one window THEN the system SHALL fold or unfold that wall's windows in the same tap that selects the wall, and SHALL mark that row with a fold indicator. A wall row with no windows SHALL carry no indicator and SHALL only select. <!-- event-driven -->
...
7. WHEN the 2D view is entered THEN the orthographic camera SHALL open at a configured half-height (5.2 m default, set by eye), or at the fit-to-room size scaled by the pull-back (1.25x) when that is larger, so a room too big for the configured size is still framed whole. Either way the opening is clamped into the same limits the pinch obeys. <!-- event-driven -->
